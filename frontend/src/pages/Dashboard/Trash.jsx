...
const Trash = () => {
  ...
  const fetchFolders = async () => {
    try {
      const response = await axios.get(`${api}folders/trash/get-folders`, {
        headers: {
  ...
  const fetchData = async () => {
    if (token) {
      try {
        const res = await axios.get(`${api}files/trash/get-files`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleFolderRestore = async (name, folderId) => {
    try {
      const response = await fetch(`${api}folders/restore/${folderId}`, {
        method: "PUT",
      });
  ...
  const handleRestoreFile = async (filename, name) => {
    try {
      const response = await fetch(`${api}files/restorefile/${filename}`, {
        method: "PUT",
      });
