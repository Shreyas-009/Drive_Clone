...
exports.secureFile = async (req, res) => {
  try {
    ...
    const file = await File.findOne({ storedName: filename, userId: userId });

    if (!file) {
      return res.status(404).json({ message: "File not found" });
    }

    const existingEmails = file.shared.map((sharedUser) => sharedUser.email);
    const newUserIdEmailPairs = userIdEmailPairs.filter(
      (userPair) => !existingEmails.includes(userPair.email)
    );

    if (newUserIdEmailPairs.length === 0) {
      return res.status(400).json({
        message: "All users provided are already shared on this file.",
      });
    }

    // Add the new users to the shared array
    file.shared.push(...newUserIdEmailPairs);
